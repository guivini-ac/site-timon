...
export function BulkActions({
  ...
  const handleAction = (action: BulkAction) => {
    if (action.requiresConfirmation) {
      ...
      const description = action.confirmationDescription || 
        `Tem certeza que deseja executar esta ação em ${selectedIds.length} ${itemName}${selectedIds.length !== 1 ? 's' : ''} selecionado${selectedIds.length !== 1 ? 's' : ''}?`;
